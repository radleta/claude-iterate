...
export function initCommand(): Command {
  return new Command('init')
    ...
    .option(
      ...
      'Stop after N consecutive no-work iterations (0=never, default: 2)',
      parseInt
    )
    ...
    .option(
      ...
      'Comma-separated events: setup_complete,execution_start,iteration,iteration_milestone,completion,error,status_update,all'
    )
    .action(
      async (
        name: string,
        ...
        try {
          ...
          // Parse mode
          const mode =
            options.mode?.toLowerCase() === 'iterative'
          ...
          const workspace = await Workspace.init(name, workspacePath, {
            ...
            notifyEvents: notifyEvents as Array<
              | 'setup_complete'
              | 'execution_start'
              | 'iteration'
              | 'iteration_milestone'
          ...
          if (
            runtimeConfig.claudeArgs.length === 0 ||
            !runtimeConfig.claudeArgs.includes('--dangerously-skip-permissions')
          ) {
            logger.info('⚠️  Claude Permission Model:');
            logger.log('');
            logger.log(
              'Claude Code will prompt for permissions during execution.'
            );
            logger.log(
              'This is the SAFE default, but may interrupt autonomous iteration.'
            );
            logger.log('');
            logger.log('To skip permission prompts (use with caution):');
            logger.log(
              '  • Per-run: claude-iterate run ' +
                name +
                ' --dangerously-skip-permissions'
            );
            logger.log(
              '  • Per-project: claude-iterate config claude.args --add --dangerously-skip-permissions'
            );
            logger.log(
              '  • Globally: claude-iterate config --global claude.args --add --dangerously-skip-permissions'
            );
            logger.log('');
